...
import { View, Text, TextInput, Button, StyleSheet, KeyboardAvoidingView, Platform, TouchableOpacity, Modal, FlatList, Keyboard, Alert } from 'react-native';
import { AuthContext } from './AuthContext';
import { useRealm } from '@realm/react';
import { BSON } from 'realm';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { IncomeEntry, Transaction, User } from './UserSchema';
...
const AddIncomeScreen = ({ route, navigation }) => {
  ...
  const bottomStyle = keyboardStatus === 'Keyboard Shown' ? { bottom: 390, height:'10%' } : { bottom: 390 };
...
// Function to handle category button press
 const handleChooseCategory = () => {
  ...
  const handeCategoryPress = async (item) => {
    ...
    const selectedCategory = currentUser.incomeCategories.filtered(`name == "${item.title}"`)[0];
    ...
    const incomeValue = parseFloat(inputValue);
    ...
    const incomeDate = DateTime.fromFormat(selectedDay, 'cccc, dd LLLL yyyy').toISO();

    try {
      realm.write(() => {
        const bson = new BSON.ObjectId();
        (selectedCategory as any).income.push({
          id: bson,
          amount: incomeValue,
          date: incomeDate,
        ...
        const newTransaction = createTransaction({
          ...
          category: item.title,
          description: notesValue,
          type: 'income',  // Since this is an expense entry
        });
    ...
    //showNotification('Income added', `Income added to ${item.title} with value of ${incomeValue} EUR`)
    Toast.show({
      type: ALERT_TYPE.SUCCESS,
      title: `${item.title} Income Added`,
      textBody: `${incomeValue} ${currency}`,
  });

    console.log(`Income added to ${item.title}: ${incomeValue} |Description: ${notesValue} Date: ${incomeDate}`);
    navigation.goBack();   
...
keypadButtonText: {
  ...
  fontFamily: 'Quicksand-Bold', // Font family
...
notesInput: {
  ...
  fontFamily: 'Quicksand-Medium', // Set the font family
  fontStyle: 'italic', // If you want the italic style as in the image
